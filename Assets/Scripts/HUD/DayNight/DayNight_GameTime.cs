...
public class DayNight_GameTime : MonoBehaviour
{
    ...
    {
        if (timer > seasons[0].dayStart * timeMultiplier && timer < seasons[0].nightStart * timeMultiplier)
        { // day		
            sunRotationZ += degreesTickSun;
            sun.transform.rotation = Quaternion.Euler(0, 180, -45 + sunRotationZ);
            if (timer > seasons[0].dayStart * timeMultiplier && timer < seasons[0].duskStart * timeMultiplier)
            {//day
                currentPhase = DayPhases.Day;
                changeColor = true;
            }
            else if (timer > seasons[0].duskStart * timeMultiplier && timer < seasons[0].nightStart * timeMultiplier)
            {//dusk		
        ...
        else if (timer > seasons[0].nightStart * timeMultiplier && timer < maxTime)
        {// night
            moonRotationZ += degreesTickMoon;
            moon.transform.rotation = Quaternion.Euler(0, 180, -45 + moonRotationZ);
        ...
        if (currentPhase != tempCurrentPhase)
        { // Avoide saving playerprefs every second
