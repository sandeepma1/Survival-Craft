...
public class LoadMapFromSave_PG : MonoBehaviour
{
    ...
    private string[][,] mapItemsFromSave;
    private sbyte[][,] mapTilesFromSave;
    private item[][,] mapItemGO;
    private int mapSize = 0;
    private int[] chunkMapSize;
    private string spriteName = "";
    private int tempPlayerPosX, tempPlayerPosY;
    ...
    private void SetSortingNamesForItems()
    {
        for (int i = 0; i < items.Length; i++)
        {
            foreach (Transform child in items[i].transform)
            {
                if (child.tag == "ItemName")
                {
                    ...
                    child.GetComponent<TextMesh>().fontSize = 100;
                    child.GetComponent<TextMesh>().color = new Color(1, 1, 1, 0);
                }
            }
        }
    }
        ...
        {    // load map tiles		
            for (int x = 0; x < chunkMapSize[i]; x++)
            {
                for (int y = 0; y < chunkMapSize[i]; y++)
                {
                    ...
                    {
                        go = Instantiate(ranTiles[Random.Range(0, ranTiles.Length)], parent);
                        //print (ranTiles [Random.Range (0, ranTiles.Length)].name);
                    }
                    else
                    {
                        go = Instantiate(tiles[mapTilesFromSave[i][x, y]], parent);
    ...
    public void InstantiateObject(GameObject go, Vector3 pos, Transform parent, int i, int id, int age)
    {
        ...
        // Random position and flip of map items
        float range = 0f;
        float randomX = Random.Range(pos.x - range, pos.x + range);
        float randomY = Random.Range(pos.y - range, pos.y + range);
        mapItemGO[i][(int)pos.x, (int)pos.y].GO.transform.localPosition = new Vector3(randomX, randomY, 0);
        if (mapItemGO[i][(int)pos.x, (int)pos.y].GO.GetComponent<SpriteRenderer>() != null)
        {
            int randomFlip = Random.Range(0, 2);
            if (randomFlip == 1)
            {
                mapItemGO[i][(int)pos.x, (int)pos.y].GO.GetComponent<SpriteRenderer>().flipX = true;
            }
        }
    ...
    public void PlayerTerrianState(int x, int y)
    {
        ...
        /*switch (mapTilesFromSave [PlayerPrefs.GetInt ("mapChunkPosition")] [(int)pos.x, (int)pos.y]) {
				case 0:
					...
					SetPlayerSpeed_n_Multiplier (PlayerMovement.Instance.speedTemp / 2, 1);
					break;
				case 1:
					GameEventManager.SetPlayerTerrianSTATES (GameEventManager.E_PlayerTerrianSTATES.water); // Player in sand
					SetPlayerSpeed_n_Multiplier (PlayerMovement.Instance.speedTemp / 1.5f, 1);
					break;		
				case 2:
					GameEventManager.SetPlayerTerrianSTATES (GameEventManager.E_PlayerTerrianSTATES.sand); // Player in sand
					SetPlayerSpeed_n_Multiplier (PlayerMovement.Instance.speedTemp, 1.5f);
					break;		
				case 3:
        ...
        /*	GameEventManager.SetPlayerTerrianSTATES (GameEventManager.E_PlayerTerrianSTATES.deepwater); //Player in deep water
			PlayerMovement.Instance.runSpeedMultiplier = 1;
			PlayerMovement.Instance.speedTemp = 0.75f;*/

        //print (GameEventManager.GetPlayerTerrianSTATES ());
    }

    private void SetPlayerSpeed_n_Multiplier(float speed, float speedMultiplier)
    {
        PlayerMovement.Instance.speed = speed;
        PlayerMovement.Instance.runSpeedMultiplier = speedMultiplier;
    }
}