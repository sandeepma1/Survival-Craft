...
{
    [AddComponentMenu("InventorySystem/Integration/DialogueSystem/Inventory manager dialogue system")]
    public class InventoryManagerDialogueSystem : MonoBehaviour
    {
        ...
        private static InventoryCurrency GetCurrencyFromID(string currencyName)
        {
            int itemID;
        ...
        {
            return InventoryManager.CanAddItem(new InventoryItemAmountRow(GetItemFromID(itemID), (uint)amount));
        }
        ...
        {
            return InventoryManager.CanRemoveCurrency(amount, GetCurrencyFromID(currencyID).ID, true);
        }
        
        private static void AddCurrency(string currencyID, float amount)
        {
            InventoryManager.AddCurrency(GetCurrencyFromID(currencyID).ID, (uint) amount);
        }

        private static void RemoveCurrency(string currencyID, float amount)
        {
            InventoryManager.RemoveCurrency(GetCurrencyFromID(currencyID).ID, (uint)amount);
        }
